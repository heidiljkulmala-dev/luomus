  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        <div className="mb-8">
          <Link href="/"><Logo /></Link>
          <h1 className="font-header text-2xl font-bold text-purple-dark mt-6">Create account</h1>
          <p className="text-sm text-muted mt-1">Join craftopia — free for all crafts</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {(["displayName", "username", "email", "password"] as const).map((field) => (
            <div key={field}>
              <label className="block text-sm font-medium text-purple-dark mb-1.5 capitalize">
        ...
        <p className="text-sm text-muted mt-6 text-center">
          Have an account?{" "}
          <Link href="/auth/sign-in" className="text-purple hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
